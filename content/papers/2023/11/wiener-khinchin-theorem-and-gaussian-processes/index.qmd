...
plt.ylabel("Signal")
ax = plt.gca()
ax.grid(axis = "x", color='black', alpha=0.15, zorder=0)
ax.grid(axis = "y", color='black', alpha=0.15, zorder=0)
plt.yticks([0])
...
plt.plot(x, clean)
plt.xlabel("x")
plt.ylabel("Signal with no noise")
ax = ax2
ax.grid(axis = "x", color='black', alpha=0.15, zorder=0)
ax.grid(axis = "y", color='black', alpha=0.15, zorder=0)
plt.yticks([0])
...
plt.axvline(16/N, color='black', alpha=0.25)
plt.axvline(30/N, color='black', alpha=0.25)
plt.semilogy(frequency[1:100], power[1:100])
...
```{python}
#| echo: false
xr = np.arange(0, N+horizon) * np.diff(x)[0]
yr = (
    sin(xr * 2 * pi * 16)       # Oscillate 16 times per interval
    + sin(xr * 2 * pi * 30)     # + 32 times per interval
)

plt.figure(figsize=(8, 2))
plt.plot(x, y, color='black', alpha=0.2, label='Noisy sample signal')
plt.plot(xr, yr, label='clean signal')
plt.plot(xr[-horizon:], forecast, label='prediction')
ax = plt.gca()
ax.grid(axis = "x", color='black', alpha=0.15, zorder=0)
ax.grid(axis = "y", color='black', alpha=0.15, zorder=0)
ax.spines['bottom'].set_color(None)
