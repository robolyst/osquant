...
From here on, we will devolatise the monthly returns before fitting a GMM. We will devolatise with a 6 month half-life. This is a hyperparameter that could be tuned. However, this article is focused on the GMM rather than devolatisation so we will not optimise this parameter.
...
We have monthly returns, we know we should devolatise them and we have decided on the number of states. Now, we can fit a GMM to the returns and investigate the results.
...
The story so far is that we have an inflationary regime (State 0), a deflationary regime (State 1) and a destressed regime (State 2).
...
However, once the model is fitted, we are limited to the sample wide set of mixing coefficients $\pi_k$. This means we are not able to interpret the states beyond how the returns behave. We need to extend this model to associate the states with other variables.
...
The next thing we want to do is estimate the state at some time $t$. Specifically, we want to estimate the probability of being in each state at time $t$. With the current model, the mixing coefficients $\pi_k$ are fixed. This means that our best estimate for the probability of being in state $k$ at time $t$ is $\pi_k$.

## Incorporating covariates

Now, let's say we have some extra information that we know at time $t$ before $\boldsymbol{r}_t$ is realised. Let's denote it with the vector $\boldsymbol{x}_t$. Now, rather than modelling $p(k)$ we can model $p(k | \boldsymbol{x}_t)$. This means that our mixing coefficients depend on the information $\boldsymbol{x}_t$ and are time-varying.
...
1. The logistic regression is solved with the `scipy.optimize.minimize` function. The solver is in a stand-alone function called `fit_multinomial_logistic` which you can also use seperately for other applications.
...
**US Treasury yields.** The 10 year yield ([GS10](https://fred.stlouisfed.org/series/GS10)), 5 year ([GS5](https://fred.stlouisfed.org/series/GS5)) and 1 year ([GS1](https://fred.stlouisfed.org/series/GS1)) are included. These yields reflect the market's expectations of future interest rates and economic growth.
![](images/treasury_yields.svg)

**VIX.** The VIX index ([VIXCLS](https://fred.stlouisfed.org/series/VIXCLS)x) is a measure of market volatility derived from S&P 500 options prices. It is often referred to as the "fear gauge" as it tends to spike during periods of market stress.
![](images/vix.svg)

**US CPI growth rate.** The CPI ([CPIAUCSL](https://fred.stlouisfed.org/series/CPIAUCSL)) growth rate is a measure of inflation. Here we show the index transformed by taking the 12-step percentage change. This shows the annual inflation rate.
![](images/cpi.svg)

**US money supply.** The M2 money ([M2SL](https://fred.stlouisfed.org/series/M2SL)) supply is the federal reserve's estimate of total liquid assets in the economy. Here we show the index transformed by taking the 12-step percentage change. This shows the annual rate of change in the money supply.
![](images/money_supply.svg)

**Unemployment rate.** The unemployment rate ([UNRATE](https://fred.stlouisfed.org/series/UNRATE)) is a measure of the percentage of people who are unemployed.
![](images/unrate.svg)
...
A multinomial logistic regression model is not very interpretable. The coefficient for state $k$ on variable $x_j$ affects all the other states due to the normalisation. We can still get an idea of the impact of each variable on each state's probability by calculating the average [marginal effect](https://bookdown.org/mike/data_analysis/sec-marginal-effects.html) (AME).
...
You will notice that the ordering of the states is different to before. This is because the states are identified in a different order each time the model is fitted. This is an indeterminancy. The important thing is that the collection of states retain their economic meaning despite the new dependency on economic variables.
...
**S&P 500.** The [S&P 500 index](https://fred.stlouisfed.org/series/SP500) level has a strong positive effect on the probability of being in the inflationary state and a strong negative effect on the probability of being in the distressed states. This makes sense as a rising stock market is part of the definition of our inflationary state.
...
**WPSID61.** This is the [producer price index for intermediate materials](https://fred.stlouisfed.org/series/WPSID61), supplies and components. It has a strong positive effect on the deflationary state and a strong negative effect on the inflationary state. This makes sense as rising input prices results in lower profit margins which can lead to lower business valuations.

**BUSINVx.** This is the [total business inventories](https://fred.stlouisfed.org/series/BUSINV). It has a strong positive effect on the inflationary state and a strong negative effect on the distressed state. This makes sense as rising inventories can be a sign of strong consumer demand leading to high profits.
